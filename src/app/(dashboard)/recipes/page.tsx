  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button onClick={() => setOpen(true)}><Plus size={16} />New Recipe</Button>}
      />
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(4)].map((_, i) => <div key={i} className="h-40 bg-[#E8DCC8] rounded-lg animate-pulse" />)}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16">
          <UtensilsCrossed size={40} className="text-[#8B7355]/30 mx-auto mb-3" />
          <p className="text-[#8B7355]">{search ? "No recipes match" : "No recipes yet"}</p>
          {!search && <Button className="mt-4" onClick={() => setOpen(true)}><Plus size={16} />Create First Recipe</Button>}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <Card key={recipe.id} className="group hover:shadow-md transition-shadow">
                <CardContent className="py-4">
                  <div className="flex items-start justify-between mb-2">
                    <Link href={`/recipes/${recipe.id}`} className="flex-1 min-w-0 cursor-pointer">
                      <h3 className="font-display font-bold text-[#2C1810] truncate group-hover:text-[#8B4513] transition-colors">{recipe.name}</h3>
                      {recipe.category && (
                        <span className="text-[10px] font-mono uppercase tracking-wider text-[#8B4513] bg-[#E8DCC8] px-1.5 py-0.5 rounded">{recipe.category}</span>
                      )}
                    </Link>
                    <div className="flex gap-1 ml-2 shrink-0">
                      {recipe.isAiGenerated && <Badge variant="pro"><Sparkles size={9} />AI</Badge>}
                      <button
                        onClick={e => { e.stopPropagation(); remove.mutate(recipe.id) }}
